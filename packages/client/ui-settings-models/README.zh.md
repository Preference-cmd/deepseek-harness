...
pi-ai profile 的 `models` 列表就在卡片上编辑：一行一个模型，行上显示 id 与显示名称，上下文窗口、输出上限、输入模态与推理档位收在该行的展开区内，右侧是两个无文字的操作——展开与删除。空列表意味着「使用该路由的内置 catalog」，因此每一行都只会被刻意添加；清空容量会丢弃它，而不是存入一个 schema 会拒绝的值，配置留空的部分由适配器的路由级回退值定尺寸——留空的容量以这些回退值的量级作为占位符，那只是提示而非镜像：该字段按 1000 计 `K`，且部署可以覆盖这些回退值。不是正整数的容量根本不会被存下。**输入模态**组是两个图标按钮（各用一个 Reicon 图标）：一个锁定的文本按钮和一个图像切换按钮；选中图像写入 `input: [text, image]`，取消选中则写入 `input: [text]`。单独的**输出模态**组只有一个锁定的文本按钮——聊天输出始终是文本，因此它只作展示、无需写入。推理档位切换按钮（`minimal` 到 `max`）会按选中的档位写入 `reasoningEfforts` 为 `{ off: null, <档位>: <档位> }`，一个都不选中时写入 `reasoningEfforts: false`——即非推理模型——而不是适配器会拒绝的空字典。
