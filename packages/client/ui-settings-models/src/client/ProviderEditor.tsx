 ...
 * control can only be set to a value some of them reject. The per-model
 * capability — the input modalities and reasoning levels a pi-ai model
 * declares — lives in the model rows this card edits, so the composer's model
 * picker offers each model its own levels; `cordis.patch.yml` keeps the profile
 * field for a deployment that knows its route. Everything else stays owned by
 * `cordis.patch.yml`. Profile edits land as minimal `settings.mutate`
