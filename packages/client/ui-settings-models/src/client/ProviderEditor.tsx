 ...
 * same reason).
 * The route-level `reasoning` default stays absent: it is a per-MODEL
 ...
 * The per-model capability itself — the input modalities and reasoning levels
 * a pi-ai model declares — lives in the model rows this card edits, so the
 * composer's model picker can then offer each model its own levels.
 * `settings.yaml` keeps every other profile field for a deployment that knows
 * its route. Profile edits land as minimal `settings.mutate`
