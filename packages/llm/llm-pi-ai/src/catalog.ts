...
/**
 * pi-ai thinking formats a profile cannot name: each drives the request
 * through template arguments — `chatTemplateKwargs` for the `chat-template`
 * variants, `chatTemplateArgs` for `baseten` — which this configuration does
 * not expose.
 */
 ...
 * key type is a drift gate: a pi-ai upgrade that adds a format (0.84's
 * `baseten`, now withheld above) fails compilation here until the format is
 * classified as offered here or withheld above, so the offer never silently
 * lags the upstream set.
 */
