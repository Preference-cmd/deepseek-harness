...
**The configuration surface now edits `input` and `reasoningEfforts`.** This reverses the original "no config surface" posture for those two fields alone: the model-list editor's per-row disclosure gained toggle chips for image input and reasoning levels ([[2026-08-14-model-row-input-and-reasoning-editor]]), because a web-only user had no way to declare either on a custom provider. The resolution chain above is unchanged — the chips write the same `input`/`reasoningEfforts` values a hand-written settings document would — and `compat`, `thinkingBudgets`, and `headers` remain settings-document-only. The row patch still spreads the stored row before applying changes, so a hand-written value survives adoption.
