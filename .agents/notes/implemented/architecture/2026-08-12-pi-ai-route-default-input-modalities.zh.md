...
**配置界面现在会编辑 `input` 与 `reasoningEfforts`。** 这单独反转了最初「无配置界面」的姿态（仅限这两个字段）：模型列表编辑器的逐行折叠区新增了图像输入与推理档位的切换按钮（[[2026-08-14-model-row-input-and-reasoning-editor]]），因为纯 Web 用户无法在自定义提供方上声明这两者。上文的解析链不变——按钮写入的正是手写 settings 文档会写入的 `input`/`reasoningEfforts` 值——而 `compat`、`thinkingBudgets`、`headers` 仍是 settings 文档专属。行 patch 依然会先展开已存储的行再应用改动，因此手写的值在采纳时仍能存活。
