...
两项 fork 侧修补，均由本次合并导致失败的关卡暴露：

- `packages/mermaid/tool-mermaid/package.json`：版本更新为 `0.1.7-rc.1`，其依赖段采用本窗口的区间策略（DSH 包用 `workspace:*`，`@deepseek-ai/cordis` 用 `workspace:~`）。`verify-npm-install-layout` 要求每个 `packages/*/*` 成员都携带家族版本，`release:verify --family dsh` 会对未携带的成员报错；该 fork 专属包在上游不存在，因此没有任何上游提交会更新它。
- `scripts/rescope-vendor.ts`：把 `packages/extensions/ui-cordis/src/client/CordisPreparingRow.tsx` 加入裸 `cordis` 词元的跳过清单。本窗口新增了这条 preparing 卡片家族，其 `PropsLocale<'cordis'>` 指向 `locales.ts` 中以 `NS = 'cordis'` 声明的 UI locale 命名空间 —— 这是一个产品键，与它的四个同族行文件完全相同。否则 `rescope-vendor:check` 会把该词元报为残留，因为改写它等于把一个 locale 命名空间当作包名解析。
...
- 已在合并后的树上验证：typecheck、lint、`hygiene`（18 门）、`doc-sync`（42 门）、`test:expected`（106 个用例）、`test:snapshot`（173 个场景中 169 个通过、2 个跳过）。
