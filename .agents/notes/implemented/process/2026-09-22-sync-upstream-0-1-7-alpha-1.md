...
- Verified on the merged tree: typecheck, lint, `doc-sync` (42 gates), `test:expected` (105 tests), `test:snapshot` (173 of 177 scenarios), and the focused unit suites for the touched packages (2628 tests). Two snapshot scenarios fail on this host only: `snapshots/acp` `escalation-approved` and `snapshots/session/fs-delete-recreate` record a bash tool result whose extra line is this host's `mavis-trash: moved to trash: …` deletion output; the merge changes neither scenario, and their recorded bytes are left unchanged.
