...
| `@deepseek-ai/dsh-tool-mermaid` | `mermaid_render` | `ctx.tools` | `tool/call`、`tool/result` | - | mermaid_render 校验图表源码并返回；网页客户端把返回的围栏源码渲染为图表，因此没有执行副作用。 |
...
<a id="deepseek-aidsh-tool-mermaid"></a>

## `@deepseek-ai/dsh-tool-mermaid`

### `mermaid_render`

校验 Mermaid 图表源码。返回图表源码；网页客户端将其渲染为图表。
...
来源：[`packages/mermaid/tool-mermaid/src/index.ts`](../packages/mermaid/tool-mermaid/src/index.ts)

mermaid_render 校验图表源码并返回；网页客户端把返回的围栏源码渲染为图表，因此没有执行副作用。
